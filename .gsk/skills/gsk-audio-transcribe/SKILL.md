...
| `-m`, `--model` | No | Transcription model to use. Options: 'whisper-1' (supports timestamps), 'gemini-3-flash-preview' (no timestamp support), 'elevenlabs_scribe_v2' (supports word timestamps & entity detection). Defaults to 'whisper-1'. Note: gemini-3-flash-preview does not support word-level or segment-level timestamps. (string, one of: whisper-1, gemini-3-flash-preview, elevenlabs_scribe_v2, default: `whisper-1`) |
