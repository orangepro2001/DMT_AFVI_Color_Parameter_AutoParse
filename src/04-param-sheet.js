...
/* Only INSPECTION (R/G/B) elements carry the values that belong in the parameter
   sheet. MASTER/SUBMASTER elements are node settings (Common, Mask Inspection,
   Chain Align, Adjust Mask, ...) and are deliberately not used, so a node
   without an INSPECTION element leaves the cell blank instead of borrowing the
   single MASTER value. */
function nodeValue(node,key){
  if(!node||!key) return {r:"",g:"",b:"",kind:"",found:false};
  const insp=node.rows.find(r=>String(r.paramKey)===String(key)&&r.kind==="INSPECTION");
  if(!insp) return {r:"",g:"",b:"",kind:"",found:false};
  return {r:insp.v[0],g:insp.v[1],b:insp.v[2],kind:"INSPECTION",found:true};
}
/* illumination axis (조명 축) - derived from LightSpec channels, not stored in any config verbatim.
   One LightSpec file holds every light: LIGHT<n> is the <Page> with Index n (20 channels). So the axis
   of a parameter sheet for LIGHT<n> comes from Page n - `SelectPage` is only the page currently
   selected on the machine and must NOT be used as the source.
   The composition is listed grouped by LED colour (CH1 CH5 CH9 are all White, only the angle differs)
   with the equipment's 1-based channel number (LightSpec @Index is 0-based, see CHANNEL_BASE).
   LightSpec does not split a light by camera colour, so the axis is one list (White / Blue / Green /
   Red) instead of the template's three colour columns; a white-only light would otherwise be lost. */
function axisFor(parsed,lightNum,model){
  const out={R:"",G:"",B:"",cols:{},groups:[],text:"",note:"",file:"",light:"",page:"",setIdx:"",selPage:""};
  ...
  const lights=lightsOfSpec(spec);
  let lt=lights.find(x=>x.pageIndex===lightNum);
  if(!lt&&lights.length){
    lt=lights[0];
    note=(note?note+" · ":"")+"no Page "+lightNum+" (= LIGHT"+lightNum+") in the file; Page "+lt.page+" used as the reference";
  }
  if(!lt){ out.note=note||("no Page data in "+spec.label); return out; }
  const enabled=lt.rows.filter(r=>r.chEnable==="1"&&Number(r.value)>0);
  out.groups=groupChannels(enabled);
  ["R","G","B"].forEach(cam=>{                                 /* kept for cross-checks / future use */
    out.cols[cam]=out.groups.filter(g=>g.color===cam);
  });
  out.text=out.groups.map(g=>g.name.toUpperCase()+" "
      +g.items.map(i=>channelName(i.ch)+":"+num(i.value,"")+"("+i.angle+"°)").join(" "))
    .join(" | ");
  const comp=out.groups.map(g=>g.name+" "+g.items.length).join(", ");
  out.file=spec.label;
  out.light=lt.lightName;
  out.page=lt.page;
  out.setIdx=lt.setIdx;
  out.selPage=lt.selPage;
  out.on=lt.on; out.count=lt.count;
  out.composition=comp||"none";
  out.note=(note?note+" · ":"")+out.file+" · "+out.light+" = Page "+out.page+"/"+(lt.pageCount||"?")
    +" ("+lt.on+"/"+lt.count+" channels on: "+(comp||"none")+") · machine selection: page "+out.selPage
...
function paramBlock(areaDef,node,dict){
  ...
  const params=fam.map(pair=>{
    ...
    return {label:label,key:key,r:v.r,g:v.g,b:v.b,state:"ok",kind:v.kind};
  });
...
function buildParamSheet(parsed,group,dict,opts,dictIndex,name){
  ...
  TEMPLATE_AREAS.forEach(a=>{
    if(rule&&rule.pn.indexOf(String(a.p))<0) return;      /* belongs to another light */
    const k=a.g+"|"+a.p+"|"+a.c;
  ...
  if(rule) sheet.notes.push("Light filter: "+rule.note+" — the template areas of the other lights are hidden.");
  else if(!blocks.length) sheet.notes.push("no parameter area of the template has data in the loaded files");
...
function paramSheetLayout(sheet,opts){
  ...
  rows.push(["조명 축",sheet.axis.text||"","","","","",""]); merges.push("B3:G3");
  const gvStart=4, gvList=sheet.gvClasses.length?sheet.gvClasses:((sheet.blocks||[]).length?["AU"]:[]);
